...
__global__ void conway_draw_kernel(Bitboard* board, Bitboard* board_2,
    const Cuda::vec2 size_bitboards, unsigned int* pixels, const Cuda::vec2 pixels_size, const Cuda::vec2 lx_ty, const Cuda::vec2 rx_by, float w_t)
{
    int px = blockDim.x * blockIdx.x + threadIdx.x;
    int py = blockDim.y * blockIdx.y + threadIdx.y;
    if (px >= pixels_size.x || py >= pixels_size.y) return;

    int pixel_idx = py * pixels_size.x + px;
    Cuda::vec2 point_vec = pixel_to_point_in_screen(Cuda::vec2(px, py), lx_ty, rx_by, pixels_size);
    point_vec += 4 * size_bitboards; // Centering

    if(point_vec.x < 0 || point_vec.y < 0) {
        pixels[pixel_idx] = 0xff808080;
        return;
    }
    if(point_vec.x >= size_bitboards.x * 8 || point_vec.y >= size_bitboards.y * 8) {
        pixels[pixel_idx] = 0xff808080;
        return;
    }
    ...
    */
    ...
    bool b000 = board[board_y * (int)size_bitboards.x + board_x] & ((Bitboard)1 << (bit_y * 8 + bit_x));
    bool b100 = board_2[board_y * (int)size_bitboards.x + board_x] & ((Bitboard)1 << (bit_y * 8 + bit_x));
    ...
    bool tpx = board[board_y  * (int)size_bitboards.x + pboard_x] & ((Bitboard)1 << (bit_y * 8 + pbit_x));
    bool tpy = board[pboard_y * (int)size_bitboards.x + board_x] & ((Bitboard)1 << (pbit_y * 8 + bit_x));
    bool tmx = board[board_y  * (int)size_bitboards.x + mboard_x] & ((Bitboard)1 << (bit_y * 8 + mbit_x));
    bool tmy = board[mboard_y * (int)size_bitboards.x + board_x] & ((Bitboard)1 << (mbit_y * 8 + bit_x));
...
extern "C" void draw_conway(Bitboard* d_board, Bitboard* d_board_2, const Cuda::vec2& size_bitboards, unsigned int* h_pixels, const Cuda::vec2& pix_size, const Cuda::vec2& lx_ty, const Cuda::vec2& rx_by, float transition)
{
    size_t pix_mem = pix_size.x * pix_size.y * sizeof(unsigned int);

    unsigned int* d_pixels;

    cudaMalloc((void**)&d_pixels, pix_mem);

    dim3 blockSize(16, 16);
    dim3 numBlocks((pix_size.x + blockSize.x - 1) / blockSize.x, (pix_size.y + blockSize.y - 1) / blockSize.y);
    conway_draw_kernel<<<numBlocks, blockSize>>>(d_board, d_board_2, size_bitboards, d_pixels, pix_size, lx_ty, rx_by, transition);
    cudaDeviceSynchronize();

    cudaMemcpy(h_pixels, d_pixels, pix_mem, cudaMemcpyDeviceToHost);

    cudaFree(d_pixels);
}

extern "C" void allocate_conway_grid(Bitboard** d_board, Bitboard** d_board_2, const Cuda::vec2& size_bitboards) {
    size_t board_sz = size_bitboards.x * size_bitboards.y * sizeof(Bitboard);
    cudaMalloc((void**)d_board, board_sz);
    cudaMalloc((void**)d_board_2, board_sz);
    // Initialize with random data
    Bitboard* h_board = (Bitboard*)malloc(board_sz);
    for (int i = 0; i < size_bitboards.x * size_bitboards.y; i+=3) {
        h_board[i] = rand();
    }
