...
__global__ void go(
    ...
    cuDoubleComplex z = seed_z; 
    cuDoubleComplex x = seed_x; 
    cuDoubleComplex c = make_cuDoubleComplex(pixel_x - width / 2, pixel_y - height / 2);
    c = cuCdiv(c, make_cuDoubleComplex(100.0, 0)); // Adjust aspect ratio
    ...
    int iterations;
    int march_count = 100;
    for(int march = 0; march < march_count; march++){
        cuDoubleComplex z_here = cuCadd(z, make_cuDoubleComplex(0, (march - march_count/2.) * 0.1));
        int iterations_this_time = 0;
        if (x_is_real && (intx == 2 || intx == 3)) {
            iterations_this_time = mandelbrot_iterations_2or3(z_here, intx, c, max_iterations, bailout_radius_sq, sq_radius);
        } else {
            iterations_this_time = mandelbrot_iterations(z_here, x, c, max_iterations, bailout_radius_sq, sq_radius);
        }
        if(iterations_this_time < max_iterations) {
            iterations += iterations_this_time;
        }
    }
    ...
    int cap = 1000;
    int capped_iterations = iterations > cap ? cap : iterations;
    colors[pixel_y * width + pixel_x] = cuda_color_lerp(0x00000000, 0xffffffff, capped_iterations / (double)cap);
}
...
extern "C" void mandelbrot_3d_render(
    ...
    go<<<numBlocks, threadsPerBlock>>>(
        ...
        max_iterations, internal_color, d_colors
    );
