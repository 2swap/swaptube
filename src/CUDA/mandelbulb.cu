...
__global__ void runRaymarch(
    ...
    int max_raymarch_iters, int max_mandelbulb_iters, 
    const int sdfID,
    uint32_t* colors){
...
extern "C" void render_raymarch(
    ...
    runRaymarch<<<block, threads>>>(wh, pos, normalize(camera), fov, lightPos, max_raymarch_iters, max_mandelbulb_iters, sdfID, d_colors);
}
