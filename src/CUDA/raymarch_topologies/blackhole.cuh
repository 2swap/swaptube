...
__device__ glm::vec4 surface_blackhole(glm::vec3 v) {
    float r2 = glm::dot(v, v);
    float w = 1.0f / (1.0f - r2);
    return glm::vec4(v.x, v.y, v.z, w);
}

static __device__ void dsurface_dv_blackhole(glm::vec3 v, glm::vec4& d_dx, glm::vec4& d_dy, glm::vec4& d_dz, float intensity) {
    float r2 = glm::dot(v, v);
    float denom = 2.0f / ((intensity - r2) * (intensity - r2));
...
// Metric tensor
//
// [ 1 + 4x^2 / (1-x^2-y^2-z^2)^4    4xy / (1-x^2-y^2-z^2)^4        4xz / (1-x^2-y^2-z^2)^4      ]
// [ 4xy / (1-x^2-y^2-z^2)^4        1 + 4y^2 / (1-x^2-y^2-z^2)^4    4yz / (1-x^2-y^2-z^2)^4      ]
// [ 4xz / (1-x^2-y^2-z^2)^4        4yz / (1-x^2-y^2-z^2)^4        1 + 4z^2 / (1-x^2-y^2-z^2)^4  ]

static __device__ void dmetric_dv_blackhole(glm::vec3 v,
                                  float dg_dx[3][3],
                                  float dg_dy[3][3],
                                  float dg_dz[3][3], float intensity) {
    float r2 = glm::dot(v, v);
    float denom = (intensity - r2);
