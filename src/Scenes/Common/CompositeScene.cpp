...
public:
    ...
    void add_scene(Scene* sc, string state_manager_name, double x, double y, double w, double h, bool subjugate_state_manager){
        if(subjugate_state_manager) {
    ...
    bool update_data_objects_check_if_changed() override {
        for (auto& swc : scenes){
            if(swc.scenePointer->update_data_objects_check_if_changed()) return true;
        }
        return false;
    }
    ...
    bool has_subscene_state_changed() const override {
        for (auto& swc : scenes){
            if(swc.scenePointer->has_subscene_state_changed()) return true;
        }
        return false;
    }
    ...
    void draw() override {
        ...
        for (auto& swc : scenes){
            int  width_int = state[swc.state_manager_name + ".w"] * w;
            int height_int = state[swc.state_manager_name + ".h"] * h;
            ...
            pix.overlay(*p, state[swc.state_manager_name + ".x"] * w, state[swc.state_manager_name + ".y"] * h);
        }
    }
    ...
    const StateQuery populate_state_query() const override {
        return StateQuery{
            swp.state_manager_name + ".x",
            swp.state_manager_name + ".y",
            swp.state_manager_name + ".w",
            swp.state_manager_name + ".h",
        };
    }
