...
public:
    PendulumGridScene(const double init_p1, const double init_p2, const double width = 1, const double height = 1) : CoordinateScene(width, height), grid(get_width(), get_height(), init_p1, init_p2) {}
    ...
    void draw() override {
        ...
        for (int y = 0; y < h; ++y) {
            for (int x = 0; x < w; ++x) {
                ...
                int color_mode1 = distance > 0.003 ? 0xff888888 : OPAQUE_BLACK;
