...
public:
    ...
    LambdaScene(const shared_ptr<LambdaExpression> lambda_str, const int width = VIDEO_WIDTH, const int height = VIDEO_HEIGHT) : Scene(width, height), le(lambda_str) { le->flush_uid_recursive(); le->set_positions();}
    ...
    void render_diagrams(){
        if(last_le != nullptr){
            ...
            last_le_pix = last_le->draw_lambda_diagram(2);
        }
        le->set_positions();
        le_pix = le->draw_lambda_diagram(2);
    }
    ...
    void draw() override {
        ...
        if(last_le == nullptr){
            /*if(le_pix == NULL)*/ render_diagrams(); 
            pixw = le_pix.w;
            pixh = le_pix.h;
            ...
            lambda_pixels = interpolated->draw_lambda_diagram(2);
        }
