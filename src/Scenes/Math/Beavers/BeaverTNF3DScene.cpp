...
extern "C" void beaver_TNF_3D_cuda(unsigned int* pixels, int w, int h, vec2 center, quat camera, float fov, vec3 lower, vec3 upper, std::vector<int> action, TuringMachine tm, float brightness_offset, /*float ancestor_offset, vec3 highlight, float highlight_intensity,*/ int max_steps);
...
void get_context(std::vector<int>& action, TuringMachine& tm, vec3& lower, vec3& upper, double x, double y, double z, int max_steps) {
    int states = 2;
    int symbols = 2;
    ivec3 scale(1);
...
void BeaverTNF3DScene::draw() {
    ...
    vec3 dir = normalize(rotate_vector(vec3(0, 0, 1), camera));
    ...
    vec3 lower;
    vec3 upper;
    get_context(action, tm, lower, upper, x, y, z, state["max_steps"]);
    ...
    beaver_TNF_3D_cuda(
        ...
        //vec3(state["highlight_x"], state["highlight_y"], state["highlight_z"]), state["highlight_intensity"],
        state["max_steps"]
    );
}

const StateQuery BeaverTNF3DScene::populate_state_query() const {
    StateQuery sq = {"fov", "q1", "qi", "qj", "qk", "camera_distance", "center_x", "center_y", "max_steps", "target_x", "target_y", "target_z", "scale_x", "scale_y", "scale_z", "ancestor_offset", "highlight_x", "highlight_y", "highlight_z", "highlight_intensity", "brightness_offset"};
    return sq;
}
