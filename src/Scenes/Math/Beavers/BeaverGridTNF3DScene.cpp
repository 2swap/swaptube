...
//extern "C" void beaver_grid_TNF_3D_cuda(unsigned int* pixels, int w, int h, vec3 pos, quat camera, float fov, vec3 target, vec3 up, float use_quat_camera, float shell_border, float core_border, vec3 scale, float ancestor_offset, int max_steps);
extern "C" void beaver_grid_TNF_3D_cuda(unsigned int* pixels, int w, int h, vec2 center, float dist, quat camera, float fov, vec3 target, float shell_border, float core_border, vec3 scale, float ancestor_offset, vec3 highlight, float highlight_intensity, int max_steps);
...
void BeaverGridTNF3DScene::draw() {
    ...
    beaver_grid_TNF_3D_cuda(
        pix.pixels.data(), pix.w, pix.h, vec2(state["center_x"], state["center_y"]),
