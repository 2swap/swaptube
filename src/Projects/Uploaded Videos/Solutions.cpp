...
void beginning() {
    ...
    state.add_transition("surfaces_opacity", "0");
    ...
    state.add_transition("y", "30");
    c4.stage_macroblock_and_render(AudioSegment(1));
    g.clear();


    state.set(std::unordered_map<std::string, std::string>{
        {"y", "3"}, {"d", "6"},
        {"q1", "0"},
        {"qi", "0"},
        {"qj", "0"},
    ...
    state.transition(std::unordered_map<std::string, std::string>{
        {"surfaces_opacity", "1"}
    });
