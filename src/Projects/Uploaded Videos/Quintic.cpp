...
shared_ptr<ThreeDimensionScene> get_part_4_title(){
    shared_ptr<LatexScene> ls = make_shared<LatexScene>("\\text{Quartic Recursion}", 1);
    shared_ptr<ThreeDimensionScene> title = make_shared<ThreeDimensionScene>();
    title->add_surface(Surface("ls"), ls);
    title->state.set({
        {"qi", ".01 {t} .5 * sin .01 * +"},
...
shared_ptr<ThreeDimensionScene> get_part_5_title(){
    shared_ptr<LatexScene> ls = make_shared<LatexScene>("\\text{Quintics, and Beyond}", 1);
    shared_ptr<ThreeDimensionScene> title = make_shared<ThreeDimensionScene>();
    title->add_surface(Surface("ls"), ls);
    title->state.set({
        {"qi", ".01 {t} .5 * sin .01 * +"},
...
void part_0(CompositeScene& cs, shared_ptr<ComplexPlotScene> cps) {
    ...
    rfs_intro->stage_macroblock(SilenceBlock(1.5), 1);
    ...
    for(int i = 0; i < 5; i++) {
        int size_factor = i==4?3:1;
        shared_ptr<LatexScene> title_scene = make_shared<LatexScene>(titles[i], 1, size_factor, size_factor*.25);
        ...
        shared_ptr<LatexScene> ls_formula = make_shared<LatexScene>(formulas[i], i==4?.6:1, size_factor*(i==3?5:1), size_factor);
    ...
    cs.stage_macroblock(CompositeBlock(FileBlock("the formulas which yield their solutions are increasingly complex."), SilenceBlock(1)), 3);
    ...
    cs.stage_macroblock(FileBlock("Today, we'll discover how permutational symmetries of solutions tell us about their polynomials,"), 5);
    cps->coefficients_to_roots();
    ...
    cps->state.begin_timer("ringspin_timer");
    cps->state.set("ringspin", "<ringspin_timer> .6 *");
    ...
    shared_ptr<CompositeScene> quintic_composite = make_shared<CompositeScene>(3, 3);
    quintic_composite->add_scene(title_scenes[4], "title_scene4", .5, .1);
    quintic_composite->add_scene(formula_scenes[4], "ls_formula4", .5);
    shared_ptr<ThreeDimensionScene> quintic_zoomin = make_shared<ThreeDimensionScene>();
    quintic_zoomin->add_surface(Surface("quintic_composite"), quintic_composite);
    cs.add_scene(quintic_zoomin, "quintic_zoomin", 1.5, .5);
    cs.state.transition(MICRO, "quintic_zoomin.x", ".5");
    cps->state.transition(MICRO, "construction_opacity", "0");
    cs.render_microblock();
    cps->state.transition(MICRO, "dot_radius", "0");
    cs.fade_all_subscenes(MICRO, 0);
    quintic_zoomin->state.transition(MICRO, "z", "1.1");
    cs.render_microblock();
    ...
    cps->state.set("construction_opacity", ".5");
    cps->transition_coefficient_opacities(MICRO, 0);
    cs.render_microblock();
    cs.render_microblock();
    cs.render_microblock();
    ...
    cs.stage_macroblock(SilenceBlock(1), 1);
    cps->state.transition(MICRO, "zero_crosshair_opacity", ".5");
    ...
    tds.render_microblock();
    tds.remove_all_subscenes_except("cps");
    cps->roots_to_coefficients();
    cps->state.transition(MICRO, {
    ...
    cps->state.transition(MACRO, "construction_opacity", "0");
    cps->state.transition(MACRO, {
        {"zoom", "0"},
        {"center_x", "0"},
        {"center_y", "0"},
    ...
    */
    ...
    quadratic = make_shared<LatexScene>(latex_color(0xff333333, "ax^" + latex_color(OPAQUE_WHITE, "3") + "+bx^2+cx+d"), .7, 1, .5);
    cs.add_scene_fade_in(MICRO, quadratic, "quadratic", .5, .2);
    cps->roots_to_coefficients();
    cps->state.transition(MICRO, {
        {"coefficient3_r", "1"},
    ...
    cs.stage_macroblock(SilenceBlock(1), 1);
    cps->transition_root_rings(MICRO, 0);
    quadratic->begin_latex_transition(MICRO, latex_color(0xff333333, "ax^3+bx^2+cx+d"));
    cs.render_microblock();
    ...
    cs.stage_macroblock(CompositeBlock(SilenceBlock(1), FileBlock("Given the coefficients,")), 3);
    cs.render_microblock();
    cps->transition_coefficient_rings(MICRO, 1);
    cs.render_microblock();
    cps->transition_coefficient_rings(MICRO, 0);
    ...
    fracs->state.transition(MACRO, {
        {"center_x", "-.48165"},
        {"center_y", "-.45802"},
        {"coefficient0_r", "-1.4 {t} .1 * cos .1 * +"},
        {"coefficient0_i", ".4 {t} .1 * sin .1 * +"},
    });
    fracs->render_microblock();
...
void part_1(CompositeScene& cs, shared_ptr<ComplexPlotScene> cps) {
    ...
    cs.stage_macroblock(CompositeBlock(FileBlock("To solve ax+b=0, we can just use algebra."), SilenceBlock(2)), 4);
    cs.render_microblock();
    cs.render_microblock();
...
void part_2(CompositeScene& cs, shared_ptr<ComplexPlotScene> cps, shared_ptr<ManifoldScene> ms) {
    ...
    cs.stage_macroblock(CompositeBlock(FileBlock("But what happens when we go up a degree?"), SilenceBlock(2.5)), 3);
    ...
    cs.render_microblock();
    function->begin_latex_transition(MICRO, "f("+latex_color(0xff0000ff, "a, b, c")+") \\rightarrow x_1");
    cs.render_microblock();
    function->begin_latex_transition(MICRO, "f(a, b, c) \\rightarrow "+latex_color(0xff00bb00, "x_1"));
    cs.render_microblock();
    ...
    cs.stage_macroblock(CompositeBlock(SilenceBlock(0.5), FileBlock("Assume we do have a continuous function that takes in a, b, and c,")), 4);
    ...
    cs.fade_subscene(MICRO, "sqrt_function", 0);
    cs.render_microblock();
    cs.remove_subscene("sqrt_function");
...
void part_3(CompositeScene& cs, shared_ptr<ComplexPlotScene> cps, shared_ptr<ManifoldScene> ms) {
    ...
    cs.stage_macroblock(CompositeBlock(SilenceBlock(1), CompositeBlock(FileBlock("Cubic polynomials are next."), SilenceBlock(1))), 3);
    cps->set_degree(3);
    cps->roots_to_coefficients();
    cps->state.transition(MACRO, {
        {"coefficient3_r", "{t} .2 * cos .5 * 1 +"},
        {"coefficient3_i", "{t} .3 * sin"},
    ...
    cps->state.set(reset);
    cs.stage_macroblock(CompositeBlock(FileBlock("This special way of cycling objects is called a commutator."), SilenceBlock(.5)), 5);
    cps->state.transition(MICRO, {
        {"coefficient0_opacity", "1"},
        {"coefficient1_opacity", "1"},
        {"coefficient2_opacity", "0"},
    ...
    ms->add_surface(Surface(glm::vec3(0, 0, 0), glm::vec3(.2, 0, 0), glm::vec3(0, 0, -.2), "label1"), label1);
    ms->add_surface(Surface(glm::vec3(0, 0, 0), glm::vec3(.2, 0, 0), glm::vec3(0, 0, -.2), "label2"), label2);
    ...
    cs.stage_macroblock(CompositeBlock(FileBlock("Let's try a different commutator on the square root..."), SilenceBlock(3)), 6);
    ms->state.set({
    ...
    ms->state.set({
        {"label5.x", "<sqrt_in_theta> 2 / cos <sqrt_in_radius> * .1 + <xshift> -"},
        {"label5.y", "0"},
        {"label5.z", "<sqrt_in_theta> 2 / sin <sqrt_in_radius> * .1 +"},
        {"label4.x", "<sqrt_in_theta> pi 2 * + 2 / cos <sqrt_in_radius> * .1 + <xshift> -"},
        {"label4.y", "0"},
        {"label4.z", "<sqrt_in_theta> pi 2 * + 2 / sin <sqrt_in_radius> * .1 +"},
    });
    ms->state.transition(MICRO, "sqrt_disappear", "0");
    cs.render_microblock();
    ms->state.transition(MICRO, "surfaces_opacity", "1");
    shared_ptr<LatexScene> label4 = make_shared<LatexScene>("1", 1, 0.2, 0.2);
    shared_ptr<LatexScene> label5 = make_shared<LatexScene>("2", 1, 0.2, 0.2);
    ms->add_surface(Surface(glm::vec3(0, 0, 0), glm::vec3(.2, 0, 0), glm::vec3(0, 0, -.2), "label4"), label4);
    ms->add_surface(Surface(glm::vec3(0, 0, 0), glm::vec3(.2, 0, 0), glm::vec3(0, 0, -.2), "label5"), label5);
    cs.render_microblock();
    ...
    cps->state.set(reset);
    cs.stage_macroblock(FileBlock("The cubic polynomial can therefore express permutations..."), 4);
    cps->coefficients_to_roots();
    cps->stage_swap(MICRO, "0", "1", false, true);
    cs.render_microblock();
    cps->stage_swap(MICRO, "2", "0", false, true);
...
void part_3p5(CompositeScene& cs, shared_ptr<ComplexPlotScene> cps, shared_ptr<ManifoldScene> ms) {
    ...
    shared_ptr<LatexScene> label1 = make_shared<LatexScene>("1", 1, 0.3, 0.3);
    shared_ptr<LatexScene> label2 = make_shared<LatexScene>("2", 1, 0.3, 0.3);
    shared_ptr<LatexScene> label3 = make_shared<LatexScene>("3", 1, 0.3, 0.3);
    ms->state.set({
        {"surfaces_opacity", "1"},
        {"label_twist", "0"},
    ...
    ms->add_surface(Surface(glm::vec3(0, .5, 0), glm::vec3(.3, 0, 0), glm::vec3(0, 0, -.3), "label1"), label1);
    cs.render_microblock();
    ms->add_surface(Surface(glm::vec3(0, .5, 0), glm::vec3(.3, 0, 0), glm::vec3(0, 0, -.3), "label2"), label2);
    cs.render_microblock();
    ms->add_surface(Surface(glm::vec3(0, .5, 0), glm::vec3(.3, 0, 0), glm::vec3(0, 0, -.3), "label3"), label3);
    cs.render_microblock();
    ...
    cs.render_microblock();
    ms->state.transition(MICRO, "label_twist", "6.283");
    ...
    cs.stage_macroblock(CompositeBlock(SilenceBlock(1), FileBlock("Let's try that again. This time, watch the bar tying them together.")), 3);
    ms->state.transition(MACRO, {
        {"qi", "{t} sin .01 *"},
    });
    cs.render_microblock();
    ms->state.transition(MICRO, {
        {"manifoldtie_x", "<sqrt_out_radius> <sqrt_out_theta> cos * <xshift> - (u) cos (v) 10 * sin * .05 * +"},
        {"manifoldtie_z", "<sqrt_out_radius> <sqrt_out_theta> sin * (u) sin (v) 10 * sin * .05 * +"},
    });
    cs.render_microblock();
    ms->state.transition(MICRO, {
        {"manifoldtie_x", "<sqrt_out_radius> <sqrt_out_theta> cos * <xshift> - (u) cos .01 * +"},
        {"manifoldtie_z", "<sqrt_out_radius> <sqrt_out_theta> sin * (u) sin .01 * +"},
    });
    cs.render_microblock();

    cs.stage_macroblock(CompositeBlock(FileBlock("One, two, undo, undo..."), SilenceBlock(.5)), 5);
    ms->state.transition(MICRO, "twist1", "1");
    cs.render_microblock();
    ms->state.transition(MICRO, "twist2", "1");
    cs.render_microblock();
    ...
    cs.stage_macroblock(FileBlock("See how it only went around the bottom pole once?"), 4);
    ms->state.transition(MACRO, {
        {"manifoldtie_x", "<sqrt_out_radius> <sqrt_out_theta> cos * <xshift> -"},
        {"manifoldtie_z", "<sqrt_out_radius> <sqrt_out_theta> sin *"},
    });
    ms->state.transition(MICRO, "manifoldcbrtpole_r", ".01");
    cs.render_microblock();
    ms->state.transition(MICRO, "manifoldcbrtpole_r", pole_r);
    cs.render_microblock();
    ms->state.transition(MICRO, "manifoldcbrtpole_r", ".01");
    cs.render_microblock();
    ms->state.transition(MICRO, "manifoldcbrtpole_r", pole_r);
    ...
    ms->state.transition(MICRO, "twist1", "0");
    cs.render_microblock();
    ms->state.transition(MICRO, "label_twist", "pi 6 *");
    ms->state.transition(MICRO, "twist2", "0");
    cs.render_microblock();
    ...
    cs.stage_macroblock(FileBlock("Only going around once means that the outputs cycle, without later being un-done!"), 1);
    ms->state.transition(MICRO, "label_twist", "pi 4 *");
    cs.render_microblock();
...
void part_4(CompositeScene& cs, shared_ptr<ComplexPlotScene> cps, shared_ptr<ManifoldScene> ms) {
    ...
    cs.stage_macroblock(FileBlock("Watch me do a nested commutator on the solutions."), 4);
    cps->construction.add(GeometricPoint(glm::vec2(0, 0), "1", .7, true));
    cs.render_microblock();
    cps->construction.add(GeometricPoint(glm::vec2(0, 0), "2", .7, true));
    cs.render_microblock();
    ...
    cs.stage_macroblock(CompositeBlock(SilenceBlock(1), FileBlock("This nested commutator has scrambled up the roots, just like a single commutator scrambled up 3 roots.")), 10);
    for(int i = 0; i < 6; i++) {
    ...
    cs.stage_macroblock(CompositeBlock(SilenceBlock(.7), CompositeBlock(FileBlock("They started like this."), SilenceBlock(.8))), 2);
    cps->stage_swap(MICRO, "0", "1", false);
    cs.render_microblock();
    ...
    shared_ptr<LatexScene> label2 = make_shared<LatexScene>("2", 1, 0.3, 0.3);
    shared_ptr<LatexScene> label3 = make_shared<LatexScene>("3", 1, 0.3, 0.3);
    ms->add_surface(Surface(glm::vec3(0, 0, 0), glm::vec3(.3, 0, 0), glm::vec3(0, 0, -.3), "label1"), label1);
    ms->add_surface(Surface(glm::vec3(0, 0, 0), glm::vec3(.3, 0, 0), glm::vec3(0, 0, -.3), "label2"), label2);
    ms->add_surface(Surface(glm::vec3(0, 0, 0), glm::vec3(.3, 0, 0), glm::vec3(0, 0, -.3), "label3"), label3);
    ms->state.set({
        {"label_twist", "pi"},
    });
    ms->state.set({
    ...
    ms->state.set({
        ...
        {"sqrt_in_radius", "<twist2> 6.283 * cos .2 * <twist3> 6.283 * sin .2 * <twist4> 6.283 * cos -.25 * .5 + + +"},
    });
    cs.render_microblock();
    ms->state.transition(MICRO, {
        {"d", "2.2"},
    ...
    cs.stage_macroblock(CompositeBlock(FileBlock("The workaround is comically simple. Any guesses?"), SilenceBlock(.5)), 1);
    ms->state.transition(MICRO, {
        {"d", "3.5"},
    });
    cs.render_microblock();
    ...
    ms->add_manifold("tie2",
        "<cbrt_out_radius> <cbrt_out_theta> cos * <cbrt_center_x> +", "30", "<cbrt_out_radius> <cbrt_out_theta> sin *",
        "0.00001", "0.00001",
        "0", "6.28318", "100",
        "-1", "1", "600"
    );
    ms->state.transition(MICRO, {
        ...
        {"manifoldtie_y", "(v) 1 + 2 / <cbrt_center_y> <sqrt_center_y> - * <sqrt_center_y> +"},
        {"manifoldtie2_x", "<cbrt_out_radius> <cbrt_out_theta> cos * <cbrt_center_x> +"},
        {"manifoldtie2_y", "(v) 1 + 2 / <qrt_center_y> <cbrt_center_y> - * <cbrt_center_y> +"},
    });
    cs.render_microblock();
    ...
    cs.stage_macroblock(FileBlock("With two nested root operators, we can represent a simple commutator on 3 solutions."), 3);
    ...
    cs.stage_macroblock(FileBlock("With three nested root operators, we can represent a commutator of commutators on 4 solutions."), 3);
    ...
    ms->state.set({
        ...
        {"qnt_in_radius", ".5"},
        {"hide_qnt", "0"},
        {"qnt_center_x", "<xshift> 3 *"},
        {"qnt_center_y", "<yshift> 3 * <hide_qnt> 6 * +"},
    });
    ms->add_manifold("qnt_in",
        point_x_start_qrt + " <qnt_center_x> +", "30", point_z_start_qrt,
        "0.00001", "0.00001",
        "0", "3.14159", "200",
        "-3.14159", "3.14159", "200"
    );
    ms->add_manifold("qntpole",
        "(u) cos .02 * <qnt_center_x> +", "30", "(u) sin .02 *",
        pole_r, pole_i,
        "0", "6.28318", "400",
        "-.4", ".4", "400"
    );
    ms->state.transition(MICRO, {
        ...
        {"sqrt_center_x", "<xshift> -3 *"},
        {"sqrt_center_y", "<yshift> -3 * <hide_sqrt> 6 * +"},
        {"cbrt_center_x", "<xshift> -1 *"},
        {"cbrt_center_y", "<yshift> -1 * <hide_cbrt> 6 * +"},
        {"qrt_center_x", "<xshift> 1 *"},
        {"qrt_center_y", "<yshift> 1 * <hide_qrt> 6 * +"},

        {"manifoldqnt_y", "<qnt_center_y>"},
        {"manifoldqnt_in_y", point_y_start + " <qnt_center_y> +"},
        {"manifoldqntpole_y", "(v) <qnt_center_y> +"},
    });
    cs.render_microblock();
    ...
    cs.render_microblock();
    cs.render_microblock();
    cs.fade_subscene(MICRO, "title5", 0);
    cs.fade_subscene(MICRO, "cps", 1);
    cs.render_microblock();
    cps->set_degree(2);
    cs.remove_all_subscenes();
...
void stage_loop_1(shared_ptr<ComplexPlotScene> cps, bool backwards) {
    cps->coefficients_to_roots();
    // Put the left root in the center,
    // put the center root on the bottom,
    // put the bottom root on the left.
    int left_root = -1;
    int center_root = -1;
    int bottom_root = -1;
    // Figure out which root is currently on the left (-1, 0), in the center (0, 0), and on the bottom (0, -1).
    for(int i = 0; i < 5; i++) {
        string root_r = cps->state.get_equation("root" + to_string(i) + "_r");
        string root_i = cps->state.get_equation("root" + to_string(i) + "_i");
        if(root_r + " " + root_i == "-1 0") left_root = i;
        if(root_r + " " + root_i == "0 0") center_root = i;
        if(root_r + " " + root_i == "0 -1") bottom_root = i;
    }

    if(!backwards) cps->state.transition(MICRO, {
        {"root" + to_string(left_root) + "_r", "0"},
        {"root" + to_string(left_root) + "_i", "0"},
        {"root" + to_string(center_root) + "_r", "0"},
        {"root" + to_string(center_root) + "_i", "-1"},
        {"root" + to_string(bottom_root) + "_r", "-1"},
        {"root" + to_string(bottom_root) + "_i", "0"},
    });

    else cps->state.transition(MICRO, {
        {"root" + to_string(bottom_root) + "_r", "0"},
        {"root" + to_string(bottom_root) + "_i", "0"},
        {"root" + to_string(left_root) + "_r", "0"},
        {"root" + to_string(left_root) + "_i", "-1"},
        {"root" + to_string(center_root) + "_r", "-1"},
        {"root" + to_string(center_root) + "_i", "0"},
    });
}

void stage_loop_2(shared_ptr<ComplexPlotScene> cps, bool backwards) {
    cps->coefficients_to_roots();
    // Put the right root on the top,
    // put the top root in the center,
    // put the center root on the right.
    int right_root = -1;
    int top_root = -1;
    int center_root = -1;
    // Figure out which root is currently on the right (1, 0), in the center (0, 0), and on the top (0, 1).
    for(int i = 0; i < 5; i++) {
        string root_r = cps->state.get_equation("root" + to_string(i) + "_r");
        string root_i = cps->state.get_equation("root" + to_string(i) + "_i");
        if(root_r + " " + root_i == "1 0") right_root = i;
        if(root_r + " " + root_i == "0 0") center_root = i;
        if(root_r + " " + root_i == "0 1") top_root = i;
    }

    if(!backwards) cps->state.transition(MICRO, {
        {"root" + to_string(right_root) + "_r", "0"},
        {"root" + to_string(right_root) + "_i", "0"},
        {"root" + to_string(top_root) + "_r", "1"},
        {"root" + to_string(top_root) + "_i", "0"},
        {"root" + to_string(center_root) + "_r", "0"},
        {"root" + to_string(center_root) + "_i", "1"},
    });

    else cps->state.transition(MICRO, {
        {"root" + to_string(top_root) + "_r", "0"},
        {"root" + to_string(top_root) + "_i", "0"},
        {"root" + to_string(center_root) + "_r", "1"},
        {"root" + to_string(center_root) + "_i", "0"},
        {"root" + to_string(right_root) + "_r", "0"},
        {"root" + to_string(right_root) + "_i", "1"},
    });
}

void stage_loop_commutator(shared_ptr<ComplexPlotScene> cps, bool backwards, bool ordering) {
    if(ordering) {
        stage_loop_1(cps, backwards);
        cps->render_microblock();
        stage_loop_2(cps, backwards);
        cps->render_microblock();
        stage_loop_1(cps, !backwards);
        cps->render_microblock();
        stage_loop_2(cps, !backwards);
...
void part_5(CompositeScene& cs, shared_ptr<ComplexPlotScene> cps, shared_ptr<ManifoldScene> ms) {
    ...
    cps->stage_macroblock(FileBlock("Now I'm going to make a doubly nested commutator with A and B."), 1);
    cps->render_microblock();
    ...
    cs.stage_macroblock(FileBlock("If we instead order our supercommutator in reverse..."), 2);
    cs.fade_all_subscenes_except(MICRO, "cps", 0);
    cs.render_microblock();
    cs.remove_all_subscenes_except("cps");
    cs.render_microblock();

    cps->stage_macroblock(SilenceBlock(4), 16);
    stage_loop_commutator(cps, false, false);
    stage_loop_commutator(cps, true, false);
    stage_loop_commutator(cps, false, true);
    stage_loop_commutator(cps, true, true);
    ...
    trail5->state.set({{"trail_x", "{cps.root2_r}"}, {"trail_y", "{cps.root2_i}"}});
    trail6->state.set({{"trail_x", "{cps.root3_r}"}, {"trail_y", "{cps.root3_i}"}});
    cs.stage_macroblock(CompositeBlock(FileBlock("All we did was B!"), SilenceBlock(1)), 1);
    cs.render_microblock();
    ...
    shared_ptr<LatexScene> chart = make_shared<LatexScene>(
        "\\begin{tabular}{cc}"
        "\\text{Single Swap} & $\\sqrt{\\cdot}$ \\\\\\\\"
        "\\\\\\\\"
        "\\phantom{\\text{Commutator}} & \\phantom{$\\sqrt{\\sqrt{\\cdot}}$} \\\\\\\\"
        "\\\\\\\\"
        "\\phantom{\\text{Double Commutator}} & \\phantom{$\\sqrt{\\sqrt{\\sqrt{\\cdot}}}$} \\\\\\\\"
        "\\\\\\\\"
        "\\phantom{\\text{Triple Commutator}} & \\phantom{$\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}$} \\\\\\\\"
        "\\\\\\\\"
        "\\phantom{\\text{Quadruple Commutator}} & \\phantom{$\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}$} \\\\\\\\"
        "\\\\\\\\"
        "\\phantom{\\text{Quintuple Commutator}} & \\phantom{$\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}}$}"
        "\\end{tabular}"
    , .8);
    ...
    chart->begin_latex_transition(MICRO,
        ...
        "\\text{Commutator} & $\\sqrt{\\sqrt{\\cdot}}$ \\\\\\\\"
        "\\\\\\\\"
        "\\text{Double Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\cdot}}}$ \\\\\\\\"
        "\\\\\\\\"
        "\\phantom{\\text{Triple Commutator}} & \\phantom{$\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}$} \\\\\\\\"
        "\\\\\\\\"
        "\\phantom{\\text{Quadruple Commutator}} & \\phantom{$\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}$} \\\\\\\\"
    ...
    chart->begin_latex_transition(MICRO,
        ...
        "\\text{Double Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\cdot}}}$ \\\\\\\\"
        "\\\\\\\\"
        "\\text{Triple Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}$ \\\\\\\\"
        "\\\\\\\\"
        "\\phantom{\\text{Quadruple Commutator}} & \\phantom{$\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}$} \\\\\\\\"
        "\\\\\\\\"
        "\\phantom{\\text{Quintuple Commutator}} & \\phantom{$\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}}$}"
    ...
    chart->begin_latex_transition(MICRO, 
        ...
        "\\text{Triple Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}$ \\\\\\\\"
        "\\\\\\\\"
        "\\text{Quadruple Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}$ \\\\\\\\"
        "\\\\\\\\"
        "\\phantom{\\text{Quintuple Commutator}} & \\phantom{$\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}}$}"
        "\\end{tabular}");
    cs.render_microblock();
    chart->begin_latex_transition(MICRO,
        ...
        "\\text{Quadruple Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}$ \\\\\\\\"
        "\\\\\\\\"
        "\\text{Quintuple Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}}$"
        "\\end{tabular}");
    cs.render_microblock();
    ...
    chart->begin_latex_transition(MICRO,
        ...
        "\\phantom{\\text{Cubic}} & \\phantom{● ● ●} & \\text{Commutator} & $\\sqrt{\\sqrt{\\cdot}}$ \\\\\\\\ \\\\\\\\"
        "\\phantom{\\text{Quartic}} & \\phantom{● ● ● ●} & \\text{Double Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\cdot}}}$ \\\\\\\\ \\\\\\\\"
        "& & \\text{Triple Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}$ \\\\\\\\ \\\\\\\\"
        "& & \\text{Quadruple Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}$ \\\\\\\\ \\\\\\\\"
        "& & \\text{Quintuple Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}}$"
        "\\end{tabular}");
    ...
    chart->begin_latex_transition(MICRO,
        "\\begin{tabular}{cccc}"
        "\\text{Quadratic} & ● ● & \\text{Single Swap} & $\\sqrt{\\cdot}$ \\\\\\\\ \\\\\\\\"
        "\\text{Cubic} & ● ● ● & \\text{Commutator} & $\\sqrt{\\sqrt{\\cdot}}$ \\\\\\\\ \\\\\\\\"
        "\\phantom{\\text{Quartic}} & \\phantom{● ● ● ●} & \\text{Double Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\cdot}}}$ \\\\\\\\ \\\\\\\\"
        "& & \\text{Triple Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}$ \\\\\\\\ \\\\\\\\"
        "& & \\text{Quadruple Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}$ \\\\\\\\ \\\\\\\\"
        "& & \\text{Quintuple Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}}$"
    ...
    chart->begin_latex_transition(MICRO,
        ...
        "\\text{Quadratic} & ● ● & \\text{Single Swap} & $\\sqrt{\\cdot}$ \\\\\\\\ \\\\\\\\"
        "\\text{Cubic} & ● ● ● & \\text{Commutator} & $\\sqrt{\\sqrt{\\cdot}}$ \\\\\\\\ \\\\\\\\"
        "\\text{Quartic} & ● ● ● ● & \\text{Double Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\cdot}}}$ \\\\\\\\ \\\\\\\\"
        "& & \\text{Triple Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}$ \\\\\\\\ \\\\\\\\"
        "& & \\text{Quadruple Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}$ \\\\\\\\ \\\\\\\\"
        "& & \\text{Quintuple Commutator} & $\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\sqrt{\\cdot}}}}}}$"
        "\\end{tabular}");
    ...
    cs.stage_macroblock(FileBlock("But as we just saw... The quintic can be scrambled by not just a 3rd-order commutator, but also a 4th-order commutator, a 5th-order commutator, and so on. No finite amount of nested roots will ever be enough."), 8);
    shared_ptr<LatexScene> quintic_blurb = make_shared<LatexScene>("\\text{Quintic} \\quad ● ● ● ● ●", .2);
    cs.add_scene_fade_in(MICRO, quintic_blurb, "quintic_blurb", .28, .49, .8);
    cs.render_microblock();
    cs.slide_subscene(MICRO, "quintic_blurb", 0, .16);
    cs.render_microblock();
    cs.render_microblock();
    cs.slide_subscene(MICRO, "quintic_blurb", 0, .18);
    cs.render_microblock();
    cs.render_microblock();
    cs.slide_subscene(MICRO, "quintic_blurb", 0, .3);
    cs.render_microblock();
    cs.render_microblock();
    cs.render_microblock();
    ...
    cs.stage_macroblock(FileBlock("With any finite number of nested square roots, cube roots, you name it,"), 5);
    cs.render_microblock();
    ms->state.transition(MICRO, "hide_sqrt", "0");
    cs.render_microblock();
    ms->state.transition(MICRO, "hide_cbrt", "0");
    ...
    ms->state.transition(MICRO, "hide_qrt", "0");
    cs.render_microblock();
    ms->state.transition(MICRO, "hide_qnt", "0");
    cs.render_microblock();
...
void ending(CompositeScene& cs, shared_ptr<ComplexPlotScene> cps){
    ...
    shared_ptr<Mp4Scene> demo = make_shared<Mp4Scene>(vector<string>{"InteractiveDemo"}, 60.0 / FRAMERATE);
    ...
    Mp4Scene prompt_typing({"TypingLittlewoodPrompt"}, 60.0 / FRAMERATE * 11);
    ...
    Mp4Scene first_try({"FirstTry"}, 60.0 / FRAMERATE * 2);
    first_try.stage_macroblock(FileBlock("and it spit out a working interactive demo on the spot!"), 1);
    first_try.render_microblock();

    Mp4Scene demo2(vector<string>{"InteractiveDemo"}, 60.0 / FRAMERATE);
    demo2.stage_macroblock(FileBlock("You can visit the link in the description to play with the parameters yourself!"), 1);
    demo2.render_microblock();

    demo2.stage_macroblock(FileBlock("I challenge you to use the demo to drag around the coefficients and try to find ways to swap different roots among themselves!"), 1);
    demo2.render_microblock();
    ...
    three_roots.stage_macroblock(FileBlock("Quickly prototyping alternative designs is easy as cake!"), 1);
    three_roots.render_microblock();
    ...
    shared_ptr<LatexScene> discount_code = make_shared<LatexScene>("\\text{Use code } \\textbf{2SWAPYT20} \\text{ for a 20\\% discount!}", 1);
    cs.stage_macroblock(FileBlock("Go to lovable.dev to start building today, and use my code 2SWAPYT20 for 20 percent off!"), 3);
    cs.render_microblock();
    cs.fade_all_subscenes_except(MICRO, "discount_code", 0.2);
    cs.add_scene_fade_in(MICRO, discount_code, "discount_code");
    cs.render_microblock();
    cs.render_microblock();
