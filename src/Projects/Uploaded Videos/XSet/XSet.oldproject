...
void intro() {
    ...
    cs.stage_macroblock_and_render(AudioSegment("This is the mandelbrot set."));
    // Zoom in on an interesting spot with a minibrot
    cs.state.microblock_transition(unordered_map<string,string>{
        {"zoom_exp", "-9"},
    });
    cs.stage_macroblock_and_render(AudioSegment("Known for its beauty at all resolutions and scales,"));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"seed_c_r", "-0.743643887037151"},
        {"zoom_exp", "-8"},
        {"seed_c_i", "0.14"},
    });
    cs.stage_macroblock_and_render(AudioSegment("and its stunning self-similarity,"));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"zoom_exp", "-0.8"},
    });
    cs.stage_macroblock_and_render(AudioSegment("it is the cornerstone example of a fractal."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"pixel_param_z", "1"},
        {"pixel_param_x", "0"},
        {"pixel_param_c", "0"},
    });
    cs.stage_macroblock_and_render(AudioSegment("What's more, if we just change our perspective, or, rather, spin our viewpanel through the fourth dimension,"));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"zoom_exp", "-1"},
    });
    cs.stage_macroblock_and_render(AudioSegment("we get a similar fractal called a Julia set."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"seed_c_r", "-.5"},
        {"seed_c_i", ".6"},
    });
    cs.stage_macroblock_and_render(AudioSegment("The Mandelbrot sets and the Julia sets are in planes orthogonal to each other in a certain 4d-space."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"seed_c_r", "-.47"},
        {"seed_c_i", ".65"},
        {"gradation", "0"},
    });
    cs.stage_macroblock_and_render(AudioSegment("This much is well-understood. And don't worry, I'll explain how all of that works in a sec."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"pixel_param_z", "0"},
        {"pixel_param_x", "1"},
        {"pixel_param_c", "0"},
    ...
    cs.stage_macroblock_and_render(AudioSegment("But before that, I wanna twist our perspective once more, into yet another orthogonal 2d-plane."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"zoom_exp", ".5"},
        {"gradation", "-3"},
    });
    cs.stage_macroblock_and_render(AudioSegment("This is another natural extension of the Mandelbrot Set which I found."));
    cs.state.microblock_transition(init);
    cs.stage_macroblock_and_render(AudioSegment("But, let's take it from the start."));
    cs.stage_macroblock_and_render(AudioSegment("The original mandelbrot set."));
    ...
    cs.stage_macroblock_and_render(AudioSegment("It lives in the complex plane."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"ms.opacity", ".15"},
        {"comp_axes.opacity", "0"},
    });
    cs.stage_macroblock(AudioSegment("The equation which generates it is surprisingly simple."), 2);
    ...
    cs.stage_macroblock(AudioSegment("Let's let C be 1 just for now. We'll try other values in a sec."), 4);
    ...
    cs.stage_macroblock_and_render(AudioSegment("Just square, and add one."));
    ...
    cs.stage_macroblock_and_render(AudioSegment("Over and over."));
    ...
    cs.stage_macroblock_and_render(AudioSegment(1));
    cs.stage_macroblock_and_render(AudioSegment("In our case, Z is blowing up towards infinity."));
    ...
    cs.stage_macroblock_and_render(AudioSegment("Let's try that again."));
    ...
    cs.stage_macroblock_and_render(AudioSegment("This time, let's let C be negative 1 instead of positive 1."));
    ...
    cs.stage_macroblock_and_render(AudioSegment("Plug in 0 to start off Z,"));
    ...
    cs.stage_macroblock_and_render(AudioSegment("iterate,"));
    ...
    cs.stage_macroblock_and_render(AudioSegment(1));
    cs.stage_macroblock(AudioSegment("and this time, we fall into a cyclic trap."), 5);
    ...
    cs.stage_macroblock_and_render(AudioSegment("Remember, we're working with Complex numbers here, not just real numbers."));
    ...
    cs.stage_macroblock_and_render(AudioSegment("C can be a number like i."));
    cs.stage_macroblock(AudioSegment("Running our simulation again, in this case it ends up stuck as well."), 9);
    ...
    cs.render_microblock();
    cs.stage_macroblock_and_render(AudioSegment("The differences in behavior here are the key."));
    cs.stage_macroblock(AudioSegment("Depending on what value of C we choose, sometimes Z will stay close to zero,"), 2);
    ...
    cs.stage_macroblock_and_render(AudioSegment("and sometimes it won't."));
    cs.fade_out_all_scenes();
    vals.begin_latex_transition("\\begin{tabular}{p{4cm}|p{4cm}|p{4cm}|p{4cm}} & z^2 + 1 & z^2 - 1 & z^2 + i \\\\\\\\ \\hline z_0 & 0 & 0 & 0 \\\\\\\\ z_1 & 1 & -1 & i \\\\\\\\ z_2 & 2 & 0 & -1+i \\\\\\\\ z_3 & 5 & -1 & -i \\\\\\\\ z_4 & 26 & 0 & -1+i \\\\\\\\ z_5 & 677 & -1 & -i \\\\\\\\ z_6 & 458330 & 0 & -1+i \\end{tabular}");
    cs.stage_macroblock_and_render(AudioSegment("That is the difference between a point inside the mandelbrot set, and outside it."));
    ...
    cs.state.microblock_transition(unordered_map<string,string>{
        {"eps.opacity", "1"},
        {"comp_axes.opacity", "0.5"},
    });
    cs.add_scene(&ms, "ms");
    cs.state.set(unordered_map<string,string>{
        {"ms.opacity", "0"},
    });
    cs.stage_macroblock_and_render(AudioSegment("If we make a plot of C in the complex plane,"));
    ...
    cs.stage_macroblock_and_render(AudioSegment("we'll paint points white if they blow up,"));
    ...
    cs.stage_macroblock_and_render(AudioSegment("and black if they don't."));
    int num_microblocks = 1000;
    cs.stage_macroblock(AudioSegment("Do that for all the points..."), num_microblocks);
    ...
    cs.stage_macroblock_and_render(AudioSegment("That gives us the characteristic shape of the Mandelbrot set."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"ms.opacity", "1"},
    });
    cs.stage_macroblock_and_render(AudioSegment("We can additionally add color to show how long it takes for the number to blow up."));
    cs.remove_scene(&eps);
    cs.remove_scene(&comp_axes);
    eqn.begin_latex_transition("z_{" + latex_text("next") + "} = z^2 + c");
    cs.stage_macroblock_and_render(AudioSegment("And that's how you get these pretty pictures."));
    eqn.begin_latex_transition("z_{" + latex_text("next") + "} = z^2 + c");
    cs.add_scene_fade_in(&eqn, "eqn");
    cs.state.microblock_transition(unordered_map<string,string>{
        {"ms.opacity", "0.2"},
    ...
    cs.stage_macroblock_and_render(AudioSegment("We not only had the free choice of what C can be, which is what we're plotting on screen,"));
    eqn.begin_latex_transition("z_{" + latex_text("next") + "} = "+latex_color(0xffff4444, "?")+"^2 + ?");
    cs.stage_macroblock_and_render(AudioSegment("but also the option to choose a starting value of Z."));
    ...
    cs.stage_macroblock_and_render(AudioSegment("Before, we always started Z at 0..."));
    ...
    cs.state.macroblock_transition(unordered_map<string,string>{
        {"seed_c_r", "-.5"},
        {"eqn.opacity", "0"},
        {"zoom_exp", "-.5"},
    });
    ...
    cs.stage_macroblock(AudioSegment("But let's start at 1 and see what happens."), 2);
    cs.render_microblock();
    cs.state.microblock_transition(unordered_map<string,string>{
        {"vals.opacity", "0"},
        {"ms.opacity", "1"},
    ...
    cs.stage_macroblock_and_render(AudioSegment("we started by fixing Z at zero,"));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"zoom_exp", "-0.5"},
        {"seed_z_r", "0"},
        {"seed_z_i", "0"},
    });
    cs.stage_macroblock_and_render(AudioSegment("and making C vary as a function of the pixel."));
    ...
    cs.stage_macroblock_and_render(AudioSegment("We get to choose C, and what Z should start at."));
    cs.stage_macroblock_and_render(AudioSegment("Since each of these has a real and imaginary component, that's 4 degrees of choice- a 4D shape."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"seed_c_r", "-1"},
        {"seed_c_i", "-.1"},
    ...
    cs.stage_macroblock_and_render(AudioSegment("Parameterizing the pixels of the screen as C yields a Mandelbrot set,"));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"pixel_param_z", "1"},
        {"pixel_param_x", "0"},
        {"pixel_param_c", "0"},
    });
    cs.stage_macroblock_and_render(AudioSegment("whereas parameterizing Z yields a Julia set."));
    cs.stage_macroblock_and_render(AudioSegment("Now, since we are looking at the space of Z right now, we can also watch what happens to Z over time throughout our simulation."));
    cs.state.set(unordered_map<string,string>{
        {"point_path_length", "0.5"},
        {"point_path_r", "0.25"},
        {"point_path_i", "0.6"},
    });
    cs.stage_macroblock_and_render(AudioSegment("Let's let this be our starting point."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"point_path_length", "1.5"},
    });
    cs.stage_macroblock_and_render(AudioSegment("Iterate z^2 + c once, and we get this new point."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"point_path_length", "8"},
    });
    ...
    cs.stage_macroblock_and_render(AudioSegment("When we start near the border, it dances around for a bit and then decides to explode."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"ms.opacity", "0.2"},
        {"point_path_length", "0"},
    });
    LatexScene eqn2("z_{" + latex_text("next") + "} = z^2 + c", 0.7, 1, .5);
    cs.add_scene_fade_in(&eqn2, "eqn2");
    cs.stage_macroblock_and_render(AudioSegment("Looking back at the original equation, there's a knob we've left unturned."));
    cs.stage_macroblock(AudioSegment("This exponent- what if, instead of squaring Z, we cube it?"), 2);
    eqn2.begin_latex_transition("z_{" + latex_text("next") + "} = z^" + latex_color(0xffff4444, "?") + " + c");
    cs.render_microblock();
    eqn2.begin_latex_transition("z_{" + latex_text("next") + "} = z^3 + c");
    cs.render_microblock();
    cs.state.microblock_transition(unordered_map<string,string>{
        {"ms.opacity", "1"},
        {"eqn2.opacity", "0"},
    });
    cs.stage_macroblock_and_render(AudioSegment("Here's what we get."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"seed_x_r", "3"},
        {"zoom_exp", "-0.3"},
    });
    ...
    cs.stage_macroblock_and_render(AudioSegment("This is a third-degree Julia set!"));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"seed_c_r", "-.58"},
        {"seed_c_i", "-.27"},
    });
    ...
    cs.stage_macroblock_and_render(AudioSegment("We can experiment with the exponent all day."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"seed_x_r", "4"},
    ...
    cs.stage_macroblock_and_render(AudioSegment("We can even make it a non-integer."));
    cs.stage_macroblock_and_render(AudioSegment(1));
    ...
    cs.stage_macroblock_and_render(AudioSegment("Heck, we can even make it a complex number too!"));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"zoom_exp", "-3.7"},
        {"seed_c_r", "-.5"},
        {"seed_c_i", "-.4"},
    ...
    cs.stage_macroblock_and_render(AudioSegment("With a purely imaginary exponent, you get these alien tendrily figures."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"seed_c_r", "0.5"},
        {"seed_c_i", "-.4"},
        {"zoom_exp", "-6"},
    });
    cs.stage_macroblock_and_render(AudioSegment("To be clear- this is still fundamentally a mandelbrot set! All I've done is change the exponent."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"seed_z_r", "0.2"},
        {"seed_z_i", "0.5"},
    });
    cs.stage_macroblock_and_render(AudioSegment("So, in some sense, I guess we really had 3 knobs to turn after all!"));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"zoom_exp", "0"},
        {"seed_z_r", "0"},
        {"seed_z_i", "0"},
    });
    cs.stage_macroblock_and_render(AudioSegment("And that, to me, raises another question."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"pixel_param_z", "0"},
        {"pixel_param_x", "1"},
        {"pixel_param_c", "0"},
    });
    cs.stage_macroblock_and_render(AudioSegment("What if we now change the pixel to represent the exponent?"));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"seed_x_r", "2"},
        {"seed_x_i", "0.5"},
    });
    cs.stage_macroblock_and_render(AudioSegment("So, this is the third orthogonal plane in a six-dimensional shape. I call it the X-Set."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"seed_c_r", "1"},
        {"seed_c_i", "0"},
    });
    cs.stage_macroblock_and_render(AudioSegment("We are now fixing Z and C, and letting X vary with the pixel."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"side_panel", "1"},
    ...
    cs.stage_macroblock_and_render(AudioSegment("I'll add panels on the right which parameterize each of the 3 variables."));
    ...
    cs.stage_macroblock(AudioSegment("These are the 3 orthogonal 2-D planes corresponding to Z, X, and C."), 3);
    ...
    cs.stage_macroblock(AudioSegment("There's one more thing which I thought was too cool not to share. Spinning back into Mandelbrot territory,"), 2);
    cs.render_microblock();
    cs.state.microblock_transition(unordered_map<string,string>{
        {"gradation", "1"},
    });
    ...
    cs.stage_macroblock_and_render(AudioSegment("if we zoom into the armpit of this miniature mandelbrot..."));
    cs.stage_macroblock_and_render(AudioSegment("It's a mini Julia set, inside the Mandelbrot set!"));
    cs.stage_macroblock_and_render(AudioSegment("Now, burn this shape into your mind and watch,"));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"zoom_exp", "-3"},
        {"pixel_param_z", "1"},
        {"pixel_param_x", "0"},
    ...
    cs.stage_macroblock_and_render(AudioSegment("as I spin into Julia land..."));
    cs.stage_macroblock_and_render(AudioSegment("it's a carbon copy of the Julia set for the mandelbrot C value which we were zoomed in on!"));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"zoom_exp", "0"},
        {"gradation", "0"},
    });
    cs.stage_macroblock_and_render(AudioSegment("The takeaway here is that these fractals are not only self-similar, but also similar among one-another."));
    cs.state.microblock_transition(unordered_map<string,string>{
    ...
    cs.stage_macroblock_and_render(AudioSegment("I don't know what I was expecting when I parameterized the Mandelbrot exponent,"));
    cs.state.microblock_transition(unordered_map<string,string>{
    ...
    cs.stage_macroblock_and_render(AudioSegment("but it sure wasn't _more julia spirals_,"));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"seed_c_r", "0.5"},
        {"seed_c_i", "-0.4"},
        {"seed_z_r", "0"},
    ...
    cs.stage_macroblock_and_render(AudioSegment("and _more mandelbrot bulbs_!"));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"seed_c_r", "0.6"},
        {"seed_c_i", "-0.2"},
    });
    cs.stage_macroblock_and_render(AudioSegment("This degree of cross-parameter self-similarity totally took me by surprise."));
    cs.state.microblock_transition(unordered_map<string,string>{
        {"seed_c_r", "0"},
        {"seed_c_i", "0"},
        {"seed_z_r", "3.55"},
