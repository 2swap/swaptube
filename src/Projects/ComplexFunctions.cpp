...
void render_video() {
    ...
    scene.state.transition(MICRO, {{"sqrt_coef", "0"}, {"sin_coef", "1"}});
    scene.render_microblock();
    scene.render_microblock();
    scene.state.transition(MICRO, {{"sin_coef", "0"}, {"cos_coef", "1"}});
    scene.render_microblock();
    scene.render_microblock();
    scene.state.transition(MICRO, {{"cos_coef", "0"}, {"exp_coef", "1"}});
    scene.render_microblock();
    scene.render_microblock();
}
