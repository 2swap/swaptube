...
void render_video() {
    ...
    stage_macroblock(CompositeBlock(SilenceBlock(2), FileBlock("Its path is pretty interesting.")), 4);
    ...
    stage_macroblock(SilenceBlock(4.5), 3);
    ...
    stage_macroblock(FileBlock("and go twice the length of its point of contact."), 3);
    ...
    stage_macroblock(FileBlock("Move the starting point, and it makes a cycle of length 4."), 2);
    bsx = 2.8, bsy = -.5;
    ...
    stage_macroblock(FileBlock("Here's the pattern."), 1);
    ...
    stage_macroblock(CompositeBlock(FileBlock("From red, it takes 8."), SilenceBlock(.3)), 2);
    obs->manager.transition(MICRO, {{"ball_start_x", "{t} 3 * sin .5 * 4 +"}, {"ball_start_y", "{t} 2 * cos .5 * 2 +"}});
    cs.render_microblock();
    cs.render_microblock();

    stage_macroblock(CompositeBlock(FileBlock("From purple, 12."), SilenceBlock(.3)), 2);
    ...
    stage_macroblock(CompositeBlock(FileBlock("The regions all match the shape of the table,"), SilenceBlock(1)), 3);
    ...
    stage_macroblock(FileBlock("Parallelograms tile the plane too, so they behave similarly."), 4);
    ...
    stage_macroblock(CompositeBlock(FileBlock("Let's try other shapes!"), SilenceBlock(6)), 4);
    ...
    stage_macroblock(FileBlock("But after two, we have this..."), 1);
    obs->manager.transition(MICRO, "ball_distance", "2");
    cs.render_microblock();

    stage_macroblock(FileBlock("The ball traces along the edge of the table."), 3);
    ...
    stage_macroblock(FileBlock("Which corner defines the length we should jump by?"), 12);
        cs.render_microblock();
        cs.render_microblock();
        cs.render_microblock();
        cs.render_microblock();
    ...
    stage_macroblock(FileBlock("It's undefined- we call it a singularity."), 1);
    cs.render_microblock();

    stage_macroblock(FileBlock("Notice how this starting point is an immediate singularity,"), 1);
    obs->manager.transition(MICRO, {{"ball_start_x", "4"}, {"ball_start_y", ".999"}, {"ball_distance", "1"}});
    cs.render_microblock();

    stage_macroblock(FileBlock("as is any point on one of these 4 lines."), 2);
    obs->manager.transition(MICRO, {{"singularity_opacity", "1"}, {"island_opacity", "0.2"}});
    obs->manager.set("singularity_depth", "1");
    cs.render_microblock();
    cs.render_microblock();

    stage_macroblock(FileBlock("Our original starting point became indeterminate on the second iteration,"), 2);
    obs->manager.transition(MICRO, {{"ball_start_y", "-2"}, {"ball_start_x", "3"}});
    cs.render_microblock();
    obs->manager.transition(MICRO, "ball_distance", "2");
    cs.render_microblock();

    stage_macroblock(FileBlock("like these points here."), 1);
    obs->manager.transition(MICRO, "singularity_depth", "2");
    cs.render_microblock();
    obs->manager.set("ball_distance", "<singularity_depth>");

    stage_macroblock(FileBlock("Here are the depth 3 singularities,"), 2);
    obs->manager.transition(MICRO, {{"ball_start_y", "-2"}, {"ball_start_x", "5"}, {"singularity_depth", "3"}});
    cs.render_microblock();
    cs.render_microblock();

    stage_macroblock(FileBlock("depth 4 singularities,"), 2);
    ...
    stage_macroblock(FileBlock("and so on."), 2);
    obs->manager.transition(MICRO, "path_opacity", "0");
