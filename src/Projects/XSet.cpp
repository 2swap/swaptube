...
void intro() {
    ...
    unordered_map<string,string> init = {
        ...
        {"zoom_exp", "0"},
        {"zoom_i", "0"},
        {"max_iterations", "40"},//TODO change to 600
        {"seed_z_r", "0"},
        {"seed_z_i", "0"},
        {"seed_x_r", "2"},
        {"seed_x_i", "0"},
    ...
    LatexScene vals("\\begin{tabular}{p{4cm}|p{4cm}} & \\textbf{c=1} \\\\\\\\ \\hline z_0 & \\\\\\\\ z_1 & \\\\\\\\ z_2 & \\\\\\\\ z_3 & \\\\\\\\ z_4 & \\\\\\\\ z_5 & \\\\\\\\ z_6 & \\end{tabular}", 0.8, 1, 0.4);
    cs.add_scene_fade_in(&eqn, "eqn");
    cs.add_scene_fade_in(&vals, "vals", 0.5, 0.2);
    cs.state_manager.microblock_transition(unordered_map<string,string>{
        {"comp_axes.opacity", "0"},
    });
    ...
    StateSliderScene sszr("[seed_z_r]", "z_r", -1, 1, .4, .07);
    StateSliderScene sszi("[seed_z_i]", "z_i", -1, 1, .4, .07);
    cs.add_scene_fade_in(&sszr, "sszr", 0.1333, 0.85, true); 
    cs.add_scene_fade_in(&sszi, "sszi", 0.1333, 0.95, true); 
    ...
    StateSliderScene sscr("[seed_c_r]", "c_r", -1, 1, .3, .1);
    StateSliderScene ssci("[seed_c_i]", "c_i", -1, 1, .3, .1);
    ...
    cs.render();
    cs.state_manager.microblock_transition(unordered_map<string,string>{
        {"point_path_r", "-.15 <t> sin +"},
        {"point_path_i", "-.6 <t> sin +"},
    });
    eqn.begin_latex_transition("z_{" + latex_text("next") + "} = z^2 + c");
    cs.inject_audio_and_render(AudioSegment(1));
    ...
    cs.inject_audio_and_render(AudioSegment("I'll add panels on the right which parameterize each of the 3 variables, and we'll move the origin around in 6-space."));
