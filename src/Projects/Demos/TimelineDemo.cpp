...
void render_video() {
    CompositeScene cs;
    vec2 sz = vec2(0.25, 0.1);
    shared_ptr<StateSliderScene> mac_f = make_shared<StateSliderScene>("{macroblock_fraction}", "\\text{macro fraction}", 0, 1  , sz);
    shared_ptr<StateSliderScene> mic_f = make_shared<StateSliderScene>("{microblock_fraction}", "\\text{micro fraction}", 0, 1  , sz);
    shared_ptr<StateSliderScene> mac_n = make_shared<StateSliderScene>("{macroblock_number}"  , "\\text{macro number}"  , 0, 10 , sz);
