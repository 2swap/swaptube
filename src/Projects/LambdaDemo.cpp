...
void intro() {
    ...
    tds.inject_audio_and_render(AudioSegment(0.5));
    ...
    ls->set_expression(term);
    tds.inject_audio(AudioSegment("This pink chunk represents the factorial function."), 3);
    tds.render();
    tds.render();
    tds.render();
    dynamic_pointer_cast<LambdaApplication>(term)->get_second()->set_color_recursive(0xffffff00);
    ls->set_expression(term);
    tds.inject_audio(AudioSegment("This yellow chunk represents the number 3."), 3);
    tds.render();
    tds.render();
    tds.render();
    term->set_color(0xff0088ff);
    ls->set_expression(term);
    tds.inject_audio(AudioSegment("And this blue chunk represents the application of the function to the number."), 3);
    tds.render();
    tds.render();
    tds.render();
    ...
    tds.inject_audio(AudioSegment("We've got one, two, three..."), 4);
    tds.render();
...
void history() {
    ...
    cs.inject_audio_and_render(AudioSegment("The term calculus, up until recently, was used to describe all sorts of logical systems."));
    ...
    cs.inject_audio_and_render(AudioSegment("It's fundamentally a system for manipulating strings of symbols,"));
    cs.inject_audio(AudioSegment("just like Algebra teaches us to manipulate numbers and pluses and equals signs."), 6);
    algebra.begin_latex_transition("x+4-1=2x+1-1");
    cs.render();
    algebra.begin_latex_transition("x+4-1=2x");
    cs.render();
    algebra.begin_latex_transition("x+3=2x");
    cs.render();
    algebra.begin_latex_transition("x-x+3=2x-x");
    cs.render();
    algebra.begin_latex_transition("3=2x-x");
    cs.render();
    ...
    LatexScene lc2("\\Bigg(\\_ \\_\\Bigg)", 0.7, VIDEO_WIDTH*.5, VIDEO_HEIGHT*.25);
    ...
    lc2.begin_latex_transition("\\Bigg(\\_ \\bigg(\\_ \\_\\bigg)\\Bigg)");
    cs.render();
    lc2.begin_latex_transition("\\Bigg(\\_ \\bigg(\\_ \\Big(\\_ \\_\\Big)\\bigg)\\Bigg)");
    cs.render();
    lc2.begin_latex_transition("\\Bigg(\\_ \\bigg(\\_ \\Big(\\_ \\big(\\_ \\_\\big)\\Big)\\bigg)\\Bigg)");
    cs.render();
    lc2.begin_latex_transition("\\Bigg(\\_ \\bigg(\\_ \\Big(\\_ \\big((\\_ \\_) \\_\\big)\\Big)\\bigg)\\Bigg)");
    cs.render();
    lc2.begin_latex_transition("\\Bigg(\\_ \\bigg(\\_ \\Big(\\_ \\big(((\\_ \\_) \\_) \\_\\big)\\Big)\\bigg)\\Bigg)");
    cs.render();
    lc2.begin_latex_transition("\\Bigg(t \\bigg(\\_ \\Big(\\_ \\big(((\\_ \\_) \\_) \\_\\big)\\Big)\\bigg)\\Bigg)");
    cs.render();
    lc2.begin_latex_transition("\\Bigg(t \\bigg(w \\Big(\\_ \\big(((\\_ \\_) \\_) \\_\\big)\\Big)\\bigg)\\Bigg)");
    cs.render();
    lc2.begin_latex_transition("\\Bigg(t \\bigg(w \\Big(o \\big(((\\_ \\_) \\_) \\_\\big)\\Big)\\bigg)\\Bigg)");
    cs.render();
    lc2.begin_latex_transition("\\Bigg(t \\bigg(w \\Big(o \\big(((s \\_) \\_) \\_\\big)\\Big)\\bigg)\\Bigg)");
    cs.render();
    lc2.begin_latex_transition("\\Bigg(t \\bigg(w \\Big(o \\big(((s w) \\_) \\_\\big)\\Big)\\bigg)\\Bigg)");
    cs.render();
    lc2.begin_latex_transition("\\Bigg(t \\bigg(w \\Big(o \\big(((s w) a) \\_\\big)\\Big)\\bigg)\\Bigg)");
    cs.render();
    lc2.begin_latex_transition("\\Bigg(t \\bigg(w \\Big(o \\big(((s w) a) p\\big)\\Big)\\bigg)\\Bigg)");
    cs.render();
    ...
    */
    ...
    lambda_rule_abs.begin_latex_transition("(\\lambda " + latex_color(0xff8888ff, "a") + ".a)");
...
void visualize(){
    ...
    cs.inject_audio(AudioSegment("There's a ton of styles, but the one I chose is John Tromp's Lambda Diagrams."), 2);
    cs.render();
    cs.state_manager.subscene_transition(unordered_map<string, string>{
        {"rep_classic.y", "-1.5"},
        {"rep_keenan.y", "-1.5"},
    ...
    lambda_rule_app.begin_latex_transition(latex_color(0xff222222, "(\\_ \\_)"        ));
    cs.inject_audio_and_render(AudioSegment(.5));
    cs.inject_audio(AudioSegment("Lambda Abstractions, or template 2, are the horizontal bars at the top."), 2);
    cs.render();
    cs.render();
    it = LambdaExpression::Iterator(term);
    while (it.has_next()) {
    ...
    cs.inject_audio(AudioSegment("Variables touch the bars which bind them."), 3);
    rep_tromp.set_expression(term);
    cs.render();
    cs.render();
    cs.render();
...
void beta_reduction(){
    ...
    cs.inject_audio(AudioSegment("Remember that template 2 can be interpreted as a function."), 2);
    cs.render(); cs.render();
    ...
    cs.inject_audio_and_render(AudioSegment("We wanna create a strategy for evaluation that combines these interpretations."));
    ...
    cs.inject_audio_and_render(AudioSegment("and applies x to b."));
    ...
    cs.inject_audio_and_render(AudioSegment("Now, we're gonna perform what's called 'beta reduction'."));
    ...
    cs.inject_audio(AudioSegment("Ok, let's try a slightly trickier one."), 3);
    cs.state_manager.subscene_transition(unordered_map<string, string>{
        {"latex_beta.opacity", "0"},
    });
    cs.render();
    latexbeta.begin_latex_transition("\\bigg(\\Big(\\lambda x. \\big(x \\big(y x\\big)\\big)\\Big) (f f)\\bigg)");
    cs.render();
    cs.state_manager.subscene_transition(unordered_map<string, string>{
        {"latex_beta.opacity", "1"},
    });
    cs.render();
    cs.inject_audio(AudioSegment("To make it clear, this is the function, and this is the value."), 3);
    cs.render();
    latexbeta.begin_latex_transition("\\bigg("+latex_color(0xffff7777, "\\Big(\\lambda x. \\big(x \\big(y x\\big)\\big)\\Big)") + "(f f)\\bigg)");
    cs.render();
    latexbeta.begin_latex_transition("\\bigg("+latex_color(0xffff7777, "\\Big(\\lambda x. \\big(x \\big(y x\\big)\\big)\\Big) ")+latex_color(0xff0088ff, "(f f)")+"\\bigg)");
    cs.render();

    // Show substitution
    latexbeta.begin_latex_transition("\\bigg("+latex_color(0xffff7777, "\\Big(\\lambda " + latex_color(0xff00ff00, "x") + ". \\big(x \\big(y x\\big)\\big)\\Big) ")+latex_color(0xff0088ff, "(f f)")+"\\bigg)");
    cs.inject_audio_and_render(AudioSegment("Same as before, X is the variable bound by the lambda."));
    latexbeta.begin_latex_transition("\\bigg("+latex_color(0xffff7777, "\\Big(\\lambda " + latex_color(0xff00ff00, "x") + ". \\big(" + latex_color(0xffff0000, "x") + " \\big(y " + latex_color(0xffff0000, "x") + "\\big)\\big)\\Big) ")+latex_color(0xff0088ff, "(f f)")+"\\bigg)");
    ...
    // Substitution result
    latexbeta.begin_latex_transition(latex_color(0xffff7777, "\\big(" + latex_color(0xffff0000, "x") + " \\big(y " + latex_color(0xffff0000, "x") + "\\big)\\big)\\quad ")+latex_color(0xff0088ff, "(f f)"));
    cs.inject_audio_and_render(AudioSegment("Discard all the scaffolding,"));
    latexbeta.begin_latex_transition(latex_color(0xffff7777, "\\big(" + latex_color(0xff0088ff, "(f f)") + " \\big(y " + latex_color(0xff0088ff, "(f f)") + "\\big)\\big)"));
    cs.inject_audio_and_render(AudioSegment("and perform the replacement."));

    // Show final result

    latexbeta.begin_latex_transition(latex_color(0xff010101, "\\big((f f) \\big(y (f f)\\big)\\big)"));
    ...
    */
    ...
    cs.inject_audio(AudioSegment("Let's try one more, but alongside the diagram this time."), 3);
    cs.state_manager.subscene_transition(unordered_map<string, string>{
...
void currying() {
    ...
    cs.inject_audio_and_render(AudioSegment("Before, we applied the function like this..."));
    multi_argument.begin_latex_transition("\\bigg(\\big(\\lambda x y.\\ 5 * y + x\\big)\\ 5\\ 3\\bigg)");
    cs.inject_audio_and_render(AudioSegment("but now we can just drop the extra parentheses as shorthand too."));
    cs.fade_out_all_scenes();
...
void booleans() {
    ...
    true_latex.begin_latex_transition("\\big((\\lambda ab. a)\\ " + latex_color(0xffff0000, "V_1") + "\\ " + latex_color(0xff0088ff, "V_2") + "\\big)");
    ...
    true_scene.reduce();
    true_latex.begin_latex_transition("\\big((\\lambda b. " + latex_color(0xffff0000, "V_1") + ")\\ " + latex_color(0xff0088ff, "V_2") + "\\big)");
    ...
    false_latex.begin_latex_transition("\\big((\\lambda ab. b)\\ " + latex_color(0xffff0000, "V_1") + "\\ " + latex_color(0xff0088ff, "V_2") + "\\big)");
    ...
    false_scene.reduce();
    false_latex.begin_latex_transition("\\big((\\lambda b. b)\\ "+ latex_color(0xff0088ff, "V_2") + "\\big)");
    ...
    true_latex .begin_latex_transition("(\\lambda ab. a)");
    false_latex.begin_latex_transition("(\\lambda ab. b)");
    cs.inject_audio_and_render(AudioSegment("This means, these functions are fundamentally selectors."));
    ...
    LatexScene conditional(latex_text("IF_THEN_ELSE") + " = \\big(X\\ t\\ f\\big)", 0.5, VIDEO_WIDTH * .8, VIDEO_HEIGHT / 2);
    cs.add_scene_fade_in(&conditional, "conditional", 0.1, 0.3, true);
    cs.state_manager.superscene_transition(unordered_map<string, string>{
        {"true_scene.opacity", "0.4"},
        {"false_scene.opacity", "0.4"},
        {"true_latex.opacity", "0.4"},
        {"false_latex.opacity", "0.4"},
    });
    ...
    // Highlight t and f
    conditional.begin_latex_transition(latex_text("IF_THEN_ELSE") + " = \\big("+latex_color(0xff00ff00, "(\\lambda ab. a)") + "\\ "+latex_color(0xff00ff00, "t")+"\\ f\\big)");
    cs.inject_audio_and_render(AudioSegment("If X is true, then the output'll be t."));
    conditional.begin_latex_transition(latex_text("IF_THEN_ELSE") + " = \\big("+latex_color(0xffff0000, "(\\lambda ab. b)") + "\\ t"+latex_color(0xffff0000, "\\ f") + "\\big)");
    ...
    cs.inject_audio_and_render(AudioSegment("So how do we make common logic gates using these values?"));
    ...
    cs.inject_audio_and_render(AudioSegment("If the input is FALSE, we want the selected value to be TRUE."));
    cs.state_manager.subscene_transition(unordered_map<string, string>{
        {"not_help.opacity", "0"},
    });
    ...
    cs.inject_audio_and_render(AudioSegment("We're gonna use the same trick as with not, using x as a selector."));
...
void numerals() {
    CompositeScene cs;
    cs.inject_audio(AudioSegment("Ok, now how about numbers?"), 3);
    ...
    cs.inject_audio_and_render(AudioSegment("In other words, the number n is represented by applying some function f to some value x, n times over."));
    cs.inject_audio_and_render(AudioSegment(0.2));
    ...
    cs.inject_audio(AudioSegment("These numbers ARE FUNCTIONS that TAKE IN A FUNCTION AND A VARIABLE, and then iterate that function on that variable."), 5);
    cs.render();
    ...
    cs.render();
    f_zero.begin_latex_transition("(\\lambda "+latex_color(0xffff0000, "f")+latex_color(0xff0088ff, "x") + ".\\ "+latex_color(0xff00ff00, "x")+")");
    f_one .begin_latex_transition("(\\lambda "+latex_color(0xffff0000, "f")+latex_color(0xff0088ff, "x") + ".\\ "+latex_color(0xff00ff00, "(f\\ x)")+")");
    f_two .begin_latex_transition("(\\lambda "+latex_color(0xffff0000, "f")+latex_color(0xff0088ff, "x") + ".\\ "+latex_color(0xff00ff00, "(f\\ (f\\ x))")+")");
    f_n   .begin_latex_transition("(\\lambda "+latex_color(0xffff0000, "f")+latex_color(0xff0088ff, "x") + ".\\ "+latex_color(0xff00ff00, "(f^n\\ x)")+")");
    cs.render();
    cs.render();
    f_zero.begin_latex_transition("(\\lambda fx.\\ x)");
    f_one .begin_latex_transition("(\\lambda fx.\\ (f\\ x))");
    ...
    succ_gate.begin_latex_transition(latex_text("SUCC = ") + "\\bigg(\\lambda n.\\ " + latex_color(0xff0088ff, "\\Big(\\lambda fx.\\ \\_\\Big)") + "\\bigg)");
    ...
    cs.inject_audio(AudioSegment("We do this by using the numeral as a function and passing f and x into it."), 3);
    ...
    cs.state_manager.subscene_transition(unordered_map<string, string>{
        ...
        {"add_fast_scene.opacity", "0.3"},
        {"mult_scene.x", "0.333"},
        {"mult_scene.y", "0.333"},
        {"mult_scene.opacity", "0.3"},
        {"exp_scene.x", "0.533"},
        {"exp_scene.y", "0.533"},
        {"exp_scene.opacity", "0.3"},
    });
    ...
    ptp.begin_latex_transition(latex_color(0xffff0000, "+\\times+") + "(a,b,c,d) = " + latex_color(0xff0088ff, "(d^{(c^b)})^{a+c}"));
...
shared_ptr<LambdaExpression> factorial(){
    ...
    */
    ...
    cs.inject_audio_and_render(AudioSegment("Here's a typical implementation in python."));
    ...
    cs.inject_audio(AudioSegment("In other words, x is a fixed point if f(x) matches x."), 2);
    cs.render();
    cs.render();
    ...
    fac_lambda.set_expression(TF);
    cs.inject_audio_and_render(AudioSegment("the function which subtracts 1,"));
...
void reduction_graph(shared_ptr<LambdaExpression> TF3){
    ...
    cs.render();
    cs.inject_audio(AudioSegment("But I wanna take a closer look at the second node."), 2);
    cs.render();
    ...
    cs.inject_audio(AudioSegment("But... stepping back,"), 2);
    cs.render();
    cs.render();
    ...
    cs.inject_audio_and_render(AudioSegment("Going back to our tree of reductions,"));
    ...
    cs.inject_audio_and_render(AudioSegment("Here's an even weirder term."));
...
void extra(){
    ...
    cs.inject_audio_and_render(AudioSegment("_Merely writing a program in the typed Lambda Calculus proves something._"));
    ...
    cs.remove_all_scenes();
    cs.inject_audio(AudioSegment("Lisp, Haskell, and other programming languages are intrinsically based in the lambda calculus, unlike, say, C or Rust."), 9);
    ...
    cs.fade_out_all_scenes();
    cs.inject_audio_and_render(AudioSegment("The philosophical implications of these alternative models of computing run deep. They give us a fresh perspective on the structure of information and the nature of computation itself."));
    cs.inject_audio_and_render(AudioSegment(1));
}
...
void credits(){
    ...
    cs.inject_audio(AudioSegment("His clear formalizations of a lot of the things I _thought_ I understood helped me get through plenty of snags in rendering these diagrams."), 4);
    cs.render();
    ptp_scene.reduce();
    cs.render();
    ptp_scene.reduce();
