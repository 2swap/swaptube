void render_video() {
    ...
    c4.inject_audio_and_render(AudioSegment("It's about this point where this connect 4 series takes a turn for the theoretical."));
    c4.inject_audio_and_render(AudioSegment("So far we have been interested in strategies of connect 4, but now it's time to apply those strategies to actually get a structural understanding of the game."));
    c4.inject_audio_and_render(AudioSegment("First of all, you're gonna have to get comfortable imagining the game as a tree. From any starting position, we can draw a node."));
    ...
    c4.inject_audio_and_render(AudioSegment("As a result, it gets intractably large, really fast. At any position in the opening, there are 7 possible moves to make, This means,"));
    ...
    c4.inject_audio_and_render(AudioSegment("In other words, we are identifying the games which have either been won for red, won for yellow, or have completely filled without a winner as a tie."));
    c4.inject_audio_and_render(AudioSegment("Now, this is more easy to tell what's going on. In the case of this endgame, there are some paths that lead to a red victory, some paths that lead to a Yellow victory,"));
    ...
    c4.inject_audio_and_render(AudioSegment("In this board, Red has the option to claim victory right away, but Red also has the option to, for example, not notice it whatsoever and play this move, thereby permitting a Yellow victory."));
    c4.inject_audio_and_render(AudioSegment("Since we want to know who would win under perfect play, we will assume Red doesn't make this mistake, and succeeds in taking the correct move."));
