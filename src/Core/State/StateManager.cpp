...
VariableContents::VariableContents(string eq,
                     double val,
                     bool fr
...
int StateReturn::size() const {
    return map.size();
}

void StateReturn::set(const string& key, const double value) {
    map[key] = value;
}

bool StateReturn::contains(const string& key) const {
    return map.find(key) != map.end();
}

const double& StateReturn::operator[](const string& key) const {
    if (!contains(key)) {
        print();
...
bool StateReturn::operator==(const StateReturn& other) const {
...
unordered_map<string, double> StateReturn::get_map() const {
    return map;
}

StateManager::StateManager() : parent(nullptr), subjugated(false) {}

bool StateManager::contains(const string& varname) const {
...
StateSet StateManager::set(const string& variable, const string& equation) {
...
StateSet StateManager::set(const StateSet& equations) {
    StateSet ret = {};
...
void StateManager::remove(const string& variable) {
...
StateSet StateManager::transition(const TransitionType tt, const string& variable, const string& equation, const bool smooth) {
...
void StateManager::close_transitions(const TransitionType tt){
    if(tt == MICRO) close_all_transitions(in_microblock_transition);
    if(tt == MACRO) close_all_transitions(in_macroblock_transition);
}

void StateManager::set_parent(StateManager* p, const string& name) {
...
void StateManager::evaluate_all() {
...
string StateManager::get_equation_string(const string& variable) const {
    try {
...
double StateManager::get_local_value(const string& variable) const {
    try {
...
const void StateManager::set_subjugated(bool b) {
    subjugated = b;
}

const void StateManager::begin_timer(const string& timer_name) {
...
const ResolvedStateEquation StateManager::get_resolved_equation(const string& variable) const {
    return resolve_state_equation(get_local_variable(variable).equation);
}

void StateManager::evaluate_single_variable(const string& variable) {
...
VariableContents StateManager::get_local_variable(const string& variable) const {
...
double StateManager::get_parent_value(const string& variable) const {
    if(parent == nullptr)
        throw runtime_error("Parent was a nullptr while looking for [" + variable + "]");
    return parent->get_local_value(variable);
}

void StateManager::close_all_transitions(unordered_set<string>& in_transition){
...
ResolvedStateEquation StateManager::resolve_state_equation(const UnresolvedStateEquation& ueq) const {
