...
inline int coldist(int col1, int col2){return std::abs(geta(col1) - geta(col2)) +
                                              std::abs(getr(col1) - getr(col2)) +
                                              std::abs(getg(col1) - getg(col2)) +
                                              std::abs(getb(col1) - getb(col2));}
inline int rainbow(float x){return argb(255,
                                            std::sin((x+1/3.)*M_PI*2)*128.+128,
                                            std::sin((x+2/3.)*M_PI*2)*128.+128,
                                            std::sin((x     )*M_PI*2)*128.+128);}
inline int colorlerp(int col1, int col2, float w){return argb(std::round(lerp(geta(col1), geta(col2), w)),
                                                                      std::round(lerp(getr(col1), getr(col2), w)),
                                                                      std::round(lerp(getg(col1), getg(col2), w)),
                                                                      std::round(lerp(getb(col1), getb(col2), w)));}
inline std::string color_to_string(int c){return "(" + std::to_string(geta(c)) + ", " + std::to_string(getr(c)) + ", " + std::to_string(getg(c)) + ", " + std::to_string(getb(c)) + ")";}
inline void print_argb(int c){std::cout << color_to_string(c) << std::endl;}
...
inline int color_combine(int base_color, int over_color, float overlay_opacity_multiplier = 1) {
    ...
    int final_alpha = std::round(final_opacity * 255.0);
...
inline int black_to_blue_to_white(double w){
    int rainbow_part1 = std::max(0.,std::min(1.,w*2-0))*255.;
    int rainbow_part2 = std::max(0.,std::min(1.,w*2-1))*255.;
...
inline float linear_srgb_to_srgb(float x) {
    //return x;
    if (x >= 0.0031308)
...
inline int RGBtoYUV(const int rgb) {
    ...
    int y = std::clamp(static_cast<int>(std::round(y_f)), 0, 255);
    int u = std::clamp(static_cast<int>(std::round(u_f)), 0, 255);
    int v = std::clamp(static_cast<int>(std::round(v_f)), 0, 255);
...
inline int YUVtoRGB(const int yuv) {
    ...
    // Round and convert to integers
    int r = std::clamp(static_cast<int>(std::round(r_f)), 0, 255);
    int g = std::clamp(static_cast<int>(std::round(g_f)), 0, 255);
    int b = std::clamp(static_cast<int>(std::round(b_f)), 0, 255);
...
inline int HSVtoRGB(double h, double s, double v, int alpha = 255) {
    ...
    if (s == 0.0) {
        ...
        h = std::fmod(h, 1.0) * 6.0;  // Hue sector [0, 6)
    ...
    int r = std::clamp(static_cast<int>(std::round(r_f * 255.0)), 0, 255);
    int g = std::clamp(static_cast<int>(std::round(g_f * 255.0)), 0, 255);
    int b = std::clamp(static_cast<int>(std::round(b_f * 255.0)), 0, 255);
    return argb(alpha, r, g, b);
}

inline int pendulum_color_old(double angle1, double angle2) {
    angle1 += M_PI;
    double y_f = 127.5 +  64 * std::sin(angle1 + angle2);
    double u_f = 127.5 + 128 * std::sin(angle1)*std::cos(angle2);
    double v_f = 127.5 + 128 * std::sin(angle2);
...
inline int pendulum_color(double angle1, double angle2, double p1, double p2) {
    float sa1 = std::sin(angle1) + 0.000001;
    float sa2 = std::sin(angle2);
    float h = std::atan2(sa2, sa1)/6.283+1;
    float s = std::min((square(sa1) + square(sa2))*5.,1.);
    float v = 1-std::min(.1 * std::sqrt(p1*p1+p2*p2), 1.0);
    return HSVtoRGB(h, s, v);
}

inline std::string latex_color(unsigned int color, std::string text) {
    // Mask out the alpha channel
    unsigned int rgb = color & 0x00FFFFFF;

    // Convert to a hex string
    std::stringstream ss;
    ss << "\\textcolor{#" << std::hex << std::setw(6) << std::setfill('0') << rgb << "}{" << text << "}";
